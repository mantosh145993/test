                ...
                <div class="p-6 text-gray-900">
                    ...
                    <table class="table mt-4 w-full table-view">
                        ...
                        <tbody>
                            ...
                            <tr @class([ 'bg-gray-100'=> auth()->check() && $test->user && $test->user->name == auth()->user()->name,
                                ])>
                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                    {{ $loop->iteration }}
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                    {{ $test->user->name ?? 'No user' }}
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                    {{ $test->quiz->title ?? 'No quiz assigned' }}
                                </td>
                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                    {{ $test->result }} /
                                    {{ $test->quiz->questions_count ?? 'N/A' }}
