<div>
    ...
    <tr class="bg-white">
        ...
        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
            {{ $test->user->name ?? 'Guest' }} <!-- Fallback to 'Guest' if no user -->
        </td>
        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
            {{ $test->quiz->title ?? 'No quiz assigned' }} <!-- Fallback if no quiz -->
        </td>
        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
            {{ $test->result . '/' . ($test->quiz->questions_count ?? 'N/A') }} <!-- Fallback for quiz count -->
        </td>
        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
            {{ $test->ip_address ?? 'N/A' }} <!-- Fallback for IP address -->
        </td>
        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
